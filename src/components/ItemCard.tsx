...
export const FeatureIcon: React.FC<{ type: FeatureType }> = ({ type }) => {
  switch (type) {
    ...
    case 'key':
      return (
        <LuKey
          className='w-5 h-5 border rounded m-1 p-0.5 border-blue-900 text-blue-900 transition-colors'
...
export const ItemCard: React.FC<ProductCardProps> = ({ products }) => {
  return (
    <>
      {products.map((product) => (
        <div
          key={product.id}
          className='w-full max-w-65 m-5 bg-white border border-slate-200 rounded-2xl p-1 flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow duration-300'
        >
          {/* Top Media Area */}
          <div className='relative w-full aspect-square bg-[#000b2f]/20 rounded-xl p-4 flex items-center justify-center group overflow-hidden'>
            {/* Relocated Badge Indicator */}
            {product.isBestSeller && (
              <span className='absolute top-3 right-3 bg-[#000b2f] text-white text-[10px] font-semibold px-2 py-0.5 rounded-full shadow-sm z-10 select-none'>
                Best Seller
              </span>
            )}
          ...
          <div className='mt-2 flex flex-col grow ml-1 mr-1'>
            ...
            <div className='h-8 w-auto flex items-center justify-between overflow-hidden'>
