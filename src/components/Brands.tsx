...
const BrandBanner: React.FC = () => {
    // Corrected mismatched name mappings for consistency
    ...
    return (
        <div className="w-full pt-5 px-6 flex items-center justify-center shadow-2xl">
            ...
            <div className="max-w-7xl w-full flex flex-wrap items-center justify-center lg:justify-between gap-15 ">
                {brands.map((brand) => (
                    <div
                        key={brand.id}
                        className="flex-1 min-w-50 max-w-60 h-35 rounded-xl  flex items-center justify-center pt-2 pb-2 transition-all duration-300 hover:-translate-y-0.5 group"
                    >
                        <img
                            src={brand.imageUrl}
                            alt={`${brand.name} logo`}

                            className="max-h-35 w-full object-contain select-none pointer-events-none transition-all duration-300 filter group-hover:grayscale-0 opacity-80 group-hover:opacity-100"
                        />
                    </div>
                ))}
            </div>
